...
{
	//NETWORK->nDeviceCount
	int i = 0; int GatewayDeviceId = -1;
	for (i = 0; i < NETWORK->nDeviceCount; i++)
		if (NETWORK->ppstruDeviceList[i]->nDeviceType == 1 && NETWORK->ppstruDeviceList[i]->nNumOfInterface == 2)
	...
	switch (pstruEventDetails->nEventType)
	{
		case NETWORK_OUT_EVENT:
		{
			...
			{
				fp = fopen("TempLogFile.csv", "a+");
				str[0] = pstruEventDetails->pPacket->szPayload->packet[0];
				str[1] = pstruEventDetails->pPacket->szPayload->packet[1];
			}

			ptrIP_FORWARD_ROUTE route = NULL;
			NetSim_PACKET* packet = pstruEventDetails->pPacket;
			
			char thresholdvalue = '5';
			if (pstruEventDetails->nDeviceId == GatewayDeviceId && (pstruEventDetails->pPacket->nPacketId != 0) && pstruEventDetails->pPacket->szPayload->packet[0] <'5' )
			{
				if (fp)
				{
					fprintf(fp, "\n%s,%s,%u,%lld,", str,"  Dropped  ",pstruEventDetails->pPacket->nSourceId, pstruEventDetails->nPacketId);
					fclose(fp);
				}
				fprintf(stderr,"\nPacket dropped. Payload is %s\n",str);
			...
			else if (pstruEventDetails->nDeviceId == GatewayDeviceId && pstruEventDetails->pPacket->nPacketId != 0)
			{
				if (fp)
				{
					fprintf(fp, "\n%s,%s,%u,%lld,", str, "  Forwarded  ", pstruEventDetails->pPacket->nSourceId, pstruEventDetails->nPacketId);
					fclose(fp);
				}
			}
