	...
	case APPLICATION_IN_EVENT:
		{
				...
				pstruPacket->nControlDataType/100 != PROTOCOL_APPLICATION)
			{
				...
				//Saving the informations before deleting a packet
				int sourceId = pstruappinfo->destList[0];
				NETSIM_ID* nDestination = pstruappinfo->sourceList;
				int destId = pstruappinfo->sourceList[0];
				int destPort = pstruappinfo->sourcePort;
				int sourcePort = pstruappinfo->destPort;
				double time = pstruEventDetails->dEventTime;
				int nApplicationId = pstruappinfo->id;
				int packetSize = pstruEventDetails->dPacketSize;
				
				//Delete the packet
				fn_NetSim_Packet_FreePacket(pstruPacket);

				
				//Checking the conditions for response 
				//Receiver is not a sensor Sender is a sensor, 
				//App Type is Traffic_Sensing , 
				//Device is the destination of the packet 
				//& It is not a control packet
				if ((NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId - 1]->ppstruInterfaceList[0]->nInterfaceType != INTERFACE_LAN_802_15_4) && \
					(NETWORK->ppstruDeviceList[pstruappinfo->sourceList[0]]->ppstruInterfaceList[0]->nInterfaceType == INTERFACE_LAN_802_15_4) && \
					pstruappinfo->nAppType == TRAFFIC_SENSING && \
					pstruEventDetails->nDeviceId == pstruappinfo->destList[0] && \
					pstruEventDetails->nPacketId != 0)
				{
					fprintf(stderr, "\nPacket reached at Node\n");
					pstruEventDetails->dEventTime = time;
					pstruEventDetails->dPacketSize = packetSize;
					pstruEventDetails->nApplicationId = nApplicationId;
					pstruEventDetails->nDeviceId = sourceId;
					pstruEventDetails->nDeviceType = DEVICE_TYPE(sourceId);
					...
					//Generating a packet from destination (Node) to the source (Sensor)
					pstruEventDetails->pPacket = 
						fn_NetSim_Application_GeneratePacket(pstruappinfo,
							pstruEventDetails->dEventTime,
							sourceId,
							1,
							nDestination,
					...
					//Change the name of Packet (Response packet which we will send back)
					char packetname[25] = "ACK_SENSOR_APP_";
					char appId[20];
					itoa(nApplicationId, appId, 10);
					strcat(packetname, appId);
					strcpy(pstruEventDetails->pPacket->szPacketType, packetname);
